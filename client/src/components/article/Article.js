...
function Article() {
    ...
    try {
      ...
      if (res.data.message === "Comment posted") {
        ...
        setCurrentArticle((prevArticle) => ({
          ...prevArticle,
          comments: [...prevArticle.comments, commentObj],
        }));
        // Optionally, you can set a message indicating successful comment posting
  ...
  const saveModifiedArticle = async (editedArticle) => {
    const modifiedArticle = { ...currentArticle, ...editedArticle, dateOfModification: new Date() };
            ...
            <div>
              ...
              <span className="py-3">
                <small className="text-secondary me-4">
                  ...
                  Created on: {ISOtoUTC(currentArticle.dateOfCreation)}
                </small>
                <small className="text-secondary">
                  <FcClock className="fs-4" />
                  Modified on: {ISOtoUTC(currentArticle.dateOfModification)}
                </small>
              </span>
            </div>
            <div>
